class Solution:
    def isPalindrome(self, s: str) -> bool:
        ...
        while l < r:
            ...
            # if they have not met each other yet and it is not an AlphaNum, we just increment (ignore it and continue)
            while l < r and not self.AlphaNumerical(s[l]):
                l += 1
            # in python, if we want to call a function inside of a class, we need to pass the "self."
            # essentially, the function is only accessible when we are inside the class, so because we have a "brother" function,
            # we have to pass the class/object itself(self) to access this other function(AlphaNum)
            while r > l and not self.AlphaNumerical(s[r]):
                r -= 1

            # Compare them, if they are different, return False already
            if s[l].lower() != s[r].lower(): 
                return False
            l = l + 1
            r = r - 1
        # if it passes the entire while and does not goes to False, it must be true
        return True
